...
public class FriendActivityDao {
    ...
    /**
     * Get recent activities from friends with pagination support
     * @param userId The user whose friends' activities to retrieve
     * @param limit Maximum number of activities to return
     * @param offset Number of activities to skip (for pagination)
     * @return List of friend activities
     */
    public List<FriendActivity> getFriendActivities(int userId, int limit, int offset) {
        List<FriendActivity> activities = new ArrayList<>();
    ...
    /**
     * Get total count of friend activities (useful for pagination)
     * @param userId The user whose friends' activities to count
     * @return Total number of friend activities
     */
    public int getFriendActivitiesCount(int userId) {
        String query = 
            "SELECT COUNT(*) FROM friends_activity_view fav " +
            "JOIN friendships f ON (" +
            "    (f.user1_id = ? AND f.user2_id = fav.user_id) OR " +
            "    (f.user2_id = ? AND f.user1_id = fav.user_id)" +
            ") " +
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            e.printStackTrace();
            throw new RuntimeException("Failed to count friend activities", e);
        }
